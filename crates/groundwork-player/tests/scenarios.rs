...
// ---------------------------------------------------------------------------
// Full batch pipeline (Phase 3)
// ---------------------------------------------------------------------------

#[test]
fn batch_full_pipeline() {
    use groundwork_player::batch;

    let scenarios = vec![
        water_system::spring_exists(),
        water_system::water_flow_and_absorption(),
        basic_growth::seed_to_tree(),
        basic_growth::multi_species_planting(),
        ecosystem::diverse_garden(),
        camera_exploration::explore_above_and_below(),
        camera_exploration::camera_edge_cases(),
    ];

    let tmp = std::env::temp_dir().join("groundwork_batch_full");
    let result = batch::run_batch(&scenarios, &tmp);

    eprintln!("=== BATCH RESULTS ===");
    eprintln!("Runs: {}", result.summaries.len());
    eprintln!(
        "Passed: {}",
        result.summaries.iter().filter(|s| s.passed).count()
    );
    eprintln!("Clusters: {}", result.clusters.len());
    eprintln!("Skills: {}", result.skills_extracted.len());
    for skill in &result.skills_extracted {
        eprintln!("  Skill: {} (from {})", skill.name, skill.source_scenario);
    }
    eprintln!();
    eprintln!("{}", result.report);
    ...
    assert!(
        result.summaries.iter().all(|s| s.passed),
        "Some scenarios failed in batch"
    );
    // No failure clusters since all pass
    assert!(result.clusters.is_empty());
    // Should extract skills from passing scenarios
    assert!(!result.skills_extracted.is_empty());
    // Report should follow AGENTS.md format
    assert!(result.report.contains("## Observed"));
    assert!(result.report.contains("## Bugs"));
