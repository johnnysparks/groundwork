...
// Script lives in crates/groundwork-web/scripts/, wiki images go to repo_root/wiki/images/
const WEB_DIR = path.resolve(__dirname, '..');
const IMAGE_DIR = path.resolve(__dirname, '../../../wiki/images');
...
async function main() {
  ...
  try {
    // Small square viewport for tight thumbnails
    ...
    for (const sp of SPECIES) {
      ...
      // Hide UI early to prevent milestone popups appearing during growth
      await hideUI(page);
      ...
      for (let i = 0; i < sp.ticks.length; i++) {
        ...
        // Close-up but not clipping — show the whole plant with some context
